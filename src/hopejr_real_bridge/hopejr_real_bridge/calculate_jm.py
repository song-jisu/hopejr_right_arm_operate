...
def thumb_motor_to_joint(theta):
    a1 = 3.6 / 7.6
    a2 = (3.6 + 4.5 * theta) / 7.6
    return math.acos(a1) - math.acos(a2)

def thumb_joint_to_motor(thumb_val):
    a1 = 3.6 / 7.6
    cos_a2 = math.cos(math.acos(a1) - thumb_val)
    theta = (cos_a2 * 7.6 - 3.6) / 4.5
    return theta

def pip_dip_motor_to_joint(theta):
    return math.acos(2.6 / 7.6) - math.acos((2.6 + 2.25 * theta) / 7.6)

def pip_dip_joint_to_motor(pip_val):
    a1 = 2.6 / 7.6
    cos_a2 = math.cos(math.acos(a1) - pip_val)
    theta = (cos_a2 * 7.6 - 2.6) / 2.25
    return theta

def radial_ulnar_motor_to_joint(theta1, theta2):
...
def radial_ulnar_joint_to_motor(alpha, beta):
    def error(thetas):
        theta1, theta2 = thetas
        end1, end2, _, _ = bar_points(theta1, theta2)
...
if __name__ == "__main__":
    ...
    theta1, theta2 = 1, -3
    end1, end2, bar_center, bar_len = bar_points(theta1, theta2)
    alpha, beta = radial_ulnar_motor_to_joint(theta1, theta2)
